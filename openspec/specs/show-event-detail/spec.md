# show-event-detail Specification

## Purpose
按四元组 (trace_id, span_id, event_id, env_id) 取单个调用链事件完整详情，tags 含异常类名/message/SQL/HTTP 状态等根因数据（诊断链第 4 步）；四参必须来自 `show_trace_events`。
## Requirements
