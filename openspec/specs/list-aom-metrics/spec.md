# list-aom-metrics Specification

## Purpose
AOM（应用运维）指标定义发现工具：列出应用/组件/容器/进程/节点的真实 metric 名与 dimension 名，是 `query_aom_metric_data` 取数前的前置发现步骤；应用层指标用 AOM，云基础设施指标用 CES `list_ces_metrics`。
## Requirements
