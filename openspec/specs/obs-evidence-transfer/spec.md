...
The system SHALL require a prior, separately recorded approval bound to the exact serviceCode, investigationId and packageId
before an upload proceeds. Approval SHALL be granted only by a trusted control plane that the LLM cannot call, and MUST NOT be
exposed as an MCP tool.

#### Scenario: Upload without approval
- **WHEN** an upload is requested without a recorded approval for that identity
...
- **AND** approval SHALL come from a non-MCP trusted control plane
